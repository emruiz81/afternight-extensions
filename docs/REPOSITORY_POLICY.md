...
Official packages may remain unsigned during the migration window, but
non-draft maintainer publication signs selected release assets when an official
key has been configured.

Allowed states:

- `unsigned`: allowed for official hash-pinned assets during migration
- `verified`: allowed only when generated by `tools/sign_repository_assets.py`
  with `signature_algorithm: "ed25519"`, a committed `signature_key_id`, and a
  base64 detached signature
- `failed`: blocks installation

`unknown` should not be emitted by official repository CI.

The signed payload is line-based and deterministic; it binds package id,
version, asset filename, SHA-256 package hash, sorted runtime targets,
algorithm, and key id. `repository.json` must not declare `verified` directly.
The checked-in public key registry and the AfterNight app's trusted key set
must be updated together before publishing verified live entries.
