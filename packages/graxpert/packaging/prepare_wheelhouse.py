#!/usr/bin/env python3
"""Prepare the GraXpert offline wheelhouse and hashed requirements lock.
...
NumPy upper bound than the runtime currently ships, so the root GraXpert wheel
must be installed with `--no-deps` while the rest of the required distributions
are enumerated explicitly in the generated lockfile.

This script resolves that wheel set recursively with `pip download --no-deps`
so we can override a small number of upstream package pins that do not publish
compatible cp314 wheels for the AfterNight runtime targets.
"""
...
ROOT_WHEEL_PACKAGE = "graxpert"

# These packages are staged from the bundled AfterNight runtime instead of the
# GraXpert wheelhouse. `skimage` is included as a defensive alias for
...
# not part of the extension wheelhouse.
IGNORED_PACKAGES = {
    "customtkinter",
}
...
def collect_lock_entries(
    ...
    for wheel_path in sorted(wheelhouse.glob("*.whl")):
        ...
        if package_name in BUNDLED_RUNTIME_PACKAGES or package_name == ROOT_WHEEL_PACKAGE:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download the GraXpert wheelhouse and generate requirements.lock."
    )
    parser.add_argument(
        "--target",
        dest="targets",
