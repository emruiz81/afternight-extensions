...
def copy_package_source(destination, include_wheelhouse=False):
...
def write_lockfile_from_wheelhouse(lockfile, wheelhouse):
    ...
    lines = [
        "# Generated from a package-local custom/private wheelhouse.",
        "# Do not use package-local wheelhouses for public wheels available from official PyPI.",
        "",
    ]
...
def build_target(args, target):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        copy_package_source(staged_package, include_wheelhouse=True)
...
def main():
    parser = argparse.ArgumentParser(description="Build GraXpert package assets.")
    ...
    parser.add_argument(
        "--source-wheelhouse",
        type=Path,
        help="Existing custom/private wheelhouse to filter by target. Do not use for public PyPI wheels.",
    )
    parser.add_argument("--source-lockfile", type=Path, help="Existing requirements.lock to copy into staged assets")
    parser.add_argument(
        "--download-wheelhouse",
        action="store_true",
        help="Download a fresh target wheelhouse with prepare_wheelhouse.py for exceptional offline/custom assets.",
    )
    ...
    if args.source_lockfile and not (args.source_wheelhouse or args.download_wheelhouse):
        parser.error("--source-lockfile is only used with wheelhouse asset builds")
    if args.targets and not (args.source_wheelhouse or args.download_wheelhouse):
        parser.error("--target is only used with wheelhouse asset builds")

    args.output_dir = Path(args.output_dir)
    args.output_dir.mkdir(parents=True, exist_ok=True)

    try:
        if args.source_wheelhouse or args.download_wheelhouse:
            assets = [build_target(args, target) for target in (args.targets or TARGETS)]
        else:
            assets = [build_thin_asset(args)]
